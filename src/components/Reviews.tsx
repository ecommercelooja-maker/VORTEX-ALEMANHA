...
function Verified() {
  return (
    <span className="inline-flex h-4 w-4 items-center justify-center rounded-full bg-accent-dark text-white" title="Verifizierter Kauf" aria-label="Verifizierter Kauf">
        ...
        <div className={`grid gap-0.5 ${r.images.length > 1 ? "grid-cols-2" : ""}`}>
          {r.images.map((src) => (
            <div key={src} className="relative aspect-[4/5]">
              <Image src={src} alt={`Foto eingereicht von ${r.name}`} fill sizes="(min-width: 1024px) 25vw, (min-width: 640px) 50vw, 100vw" className="object-cover" />
            </div>
          ))}
        </div>
      )}
  ...
  return (
    <section id="reviews" className="border-t border-black/10 bg-surface">
      <div className="mx-auto max-w-7xl px-4 py-14">
        <h2 className="font-heading text-center text-4xl sm:text-5xl">Kundenbewertungen</h2>
        ...
        <div className="mx-auto mt-8 grid max-w-3xl gap-8 rounded-2xl border border-black/10 bg-white p-6 md:grid-cols-[200px_1fr]">
          <div className="text-center">
            <div className="font-heading text-7xl leading-none">{reviewSummary.average.toLocaleString("de-DE")}</div>
            <div className="mt-2 flex justify-center">
              <Stars value={reviewSummary.average} className="h-6 w-6" color="text-[#f5a623]" />
            </div>
            <div className="mt-1 text-sm text-black/60">Basierend auf {reviewSummary.total} Bewertungen</div>
          </div>
        ...
        <div className="mt-10">
          <h3 className="mb-3 text-sm font-semibold uppercase tracking-wide text-black/60">Kundenfotos</h3>
          <div className="flex gap-2 overflow-x-auto pb-2">
            {photos.map((src) => (
              <div key={src} className="relative h-24 w-24 shrink-0 overflow-hidden rounded-lg sm:h-28 sm:w-28">
                <Image src={src} alt="Kundenfoto" fill sizes="112px" className="object-cover" />
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="mt-8 flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex gap-2 text-sm">
            {(["all", "with", "without"] as Filter[]).map((f) => (
              ...
              >
                {f === "all" ? "Alle" : f === "with" ? "Mit Bildern" : "Ohne Bilder"}
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="mt-2 text-center text-sm text-black/60">
          {Math.min(shown, list.length)} von {list.length} Bewertungen
        </div>
        {shown < list.length && (
