# Connects a custom domain to the Vercel project "vortex-de" and points the site at it.
# Usage (PowerShell, from the project folder):
#   .\scripts\connect-domain.ps1 -Domain example.de
#
# What it does:
#   1. adds example.de and www.example.de to the Vercel project (www redirects to the apex)
#   2. sets NEXT_PUBLIC_SITE_URL=https://example.de for Production (canonical, sitemap, robots, policies)
...
Write-Host "==> Adding domains to project vortex-de ($Scope)"
vercel domains add $Domain vortex-de --scope $Scope
vercel domains add $www vortex-de --scope $Scope
